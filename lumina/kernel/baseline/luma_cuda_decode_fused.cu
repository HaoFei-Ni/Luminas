/**
 * @file luma_cuda_decode_fused.cu
...
__device__ float luma_cuda_decode_dot_w(const __half *W, const __half *x, int h, int d, int j)
{
    int i;
    float acc = 0.0f;
...
__device__ void luma_cuda_decode_gemv_qkv(const __half *x, const __half *Wq, const __half *Wk,
                                         ...
                                         float *q, float *k, float *v, float *acc)
{
    ...
    for (j = tid; j < d; j += nthreads) {
        ...
        v[j] = luma_cuda_decode_dot_w(Wv, x, h, d, j);
        acc[j] = 0.0f; /* 在线 softmax 累加器清零。 */
    }
}

/** 成对 RoPE；偶数 d 由启动器强制。 */
__device__ void luma_cuda_decode_rope(float *q, float *k, int half, int tid, int nthreads,
...
/** 部分点积写入 red[tid]，供块归约。 */
__device__ void luma_cuda_decode_partial_dot(float *red, const float *q, const __half *vec,
...
/** 在线 softmax 重标定后累加 V。 */
__device__ void luma_cuda_decode_acc_update(float *acc, const __half *vec, float rescale,
...
__device__ void luma_cuda_decode_attn_step(float *q, float *acc, float *red, float *st,
                                          ...
                                          float scale, int tid, int nthreads)
{
    ...
    luma_cuda_decode_acc_update(acc, vec, rescale, ww, d, tid, nthreads);
    __syncthreads(); /* 下一 token 步前 st/acc 必须一致。 */
}

/** token 维串行：前缀 max 不可并行。 */
__device__ void luma_cuda_decode_attn_loop(float *q, float *acc, float *red, float *st,
                                          const __half *Vt_r, const float *S_r,
                                          const __half *V_code, const __half *k_tail,
...
/** norm==0 时置零，避免除零。 */
__device__ void luma_cuda_decode_normalize(float *acc, float *st, int d, int tid, int nthreads)
{
    int j;
...
/** Wo 投影单行点积。 */
__device__ float luma_cuda_decode_dot_wo(const __half *Wo, const float *acc, int h, int d, int j)
{
    int i;
    float o = 0.0f;
...
/** 并行写 out。 */
__device__ void luma_cuda_decode_proj_out(const __half *Wo, const float *acc, __half *out,
...
/* 设备入口：共享内存布局见文件头。 */
__global__ void luma_cuda_decode_fused_kernel(
    ...
    __half *__restrict__ out)
{
    ...
    luma_cuda_decode_gemv_qkv(x, Wq, Wk, Wv, h, d, tid, nthreads, q, k, v, acc);
    __syncthreads(); /* RoPE 读 q/k 前须完成 GEMV。 */
    luma_cuda_decode_rope(q, k, d / 2, tid, nthreads, cos_q, sin_q, cos_k, sin_k);
    if (tid == 0) {
    ...
    luma_cuda_decode_normalize(acc, st, d, tid, nthreads);
    __syncthreads(); /* 投影读 acc 前归一化完成。 */
    luma_cuda_decode_proj_out(Wo, acc, out, h, d, tid, nthreads);
}

/* 主机启动器：参数包校验偶数 d 与 head-dim 上限。 */
extern "C" int luma_cuda_decode_fused(const luma_cuda_decode_fused_args_t *args)
{
    size_t shmem;
    int rc;
    ...
    luma_cuda_decode_fused_kernel<<<args->heads, args->threads_per_block, shmem,
                                    args->stream>>>(
